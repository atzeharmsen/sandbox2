...
# GENERAL CONSTANTS
OUT_PATH = "../Scenario_Analysis/evolution/"
...
EI_NOx_ARR = np.ones(len(NORM_TIME), dtype="float32")  # TODO

for i in range(len(NORM_TIME)):
    if i in [5, 6, 7]:
        EI_NOx_ARR[i] = 0
    else:
        EI_NOx_ARR[i] = 1 / 1000 + i / 1000
...
def create_time_normalization_xr(
    ...
    evolution.EI_NOx.attrs = {"long_name": "NOx emission index", "units": ""}
...
if __name__ == "__main__":
    ...
    norm_ds = create_time_normalization_xr(
        NORM_TIME, FUEL_ARR, EI_CO2_ARR, EI_H2O_ARR, EI_NOx_ARR, DIS_PER_FUEL_ARR
    )
    convert_xr_to_nc(norm_ds, "norm_NOX")
